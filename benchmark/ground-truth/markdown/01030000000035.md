...
$$
\mathbf{v}(f)(\mathbf{m}) = \mathbf{e}_i(f)(\mathbf{m})\, b^i(\mathbf{m}),
$$
...
$$
\tilde{\mathbf{e}}^i(\mathbf{e}_j)(\mathbf{m}) = \delta_j^i
$$
...
\(^1\) We cannot say if the basis vectors are orthogonal or normalized until we introduce a metric.
