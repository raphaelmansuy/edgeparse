...
\[
\frac{15 \text{ cmol}_c}{\text{kg}} \times 20\% \text{ increase} = 3 \frac{\text{cmol}_c}{\text{kg}} \quad \text{basic cations required from lime}
\]

\[
\frac{40 \text{ cmol}_c}{\text{kg}} \times 20\% \text{ increase} = 8 \frac{\text{cmol}_c}{\text{kg}} \quad \text{basic cations required from lime}
\]

Lastly, soil pH is governed by base saturation. If other factors are constant, the lower the pH, the more lime is required to achieve a desired pH. This is because at a low pH, a larger percentage of the CEC is occupied by acid cations, which requires larger amounts of lime to neutralize.
