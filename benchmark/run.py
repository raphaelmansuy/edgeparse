#!/usr/bin/env python3
"""Benchmark runner for EdgeParse.
...
from engine_registry import available_engines, display_name
from report_terminal import print_single_report
...
def print_summary(eval_data: dict) -> None:
    ...
    print(
        f"PBF  (Paragraph Bound.):  {paragraph_boundary_f1:.4f}"
        if paragraph_boundary_f1 is not None
        else "PBF:  N/A"
    )
    print(
        f"SBF  (Prose Blocks):      {prose_block_boundary_f1:.4f}"
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="edgeparse",
        help=(
            "Parser engine to benchmark. "
            "Available: " + ", ".join(available_engines()) + ". "
            "'edgeparse' uses the local Rust binary (default)."
        ),
    )
    parser.add_argument(
        "--html",
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        print_single_report(eval_data, args.engine)

        # Generate HTML report if requested
        if args.html:
            html_path = Path(args.html)
            html_path.parent.mkdir(parents=True, exist_ok=True)
            generate_html_report({args.engine: eval_data}, html_path)
